...
public class TestJavassistMain {
    ...
    private static void createProxy() throws Exception {
        ...
        CtClass cc = pool.makeClass("yuanjun.chen.proxy.StationProxy");

        // 设置接口
        CtClass interface1 = pool.get("yuanjun.chen.proxy.TicketService");
        cc.setInterfaces(new CtClass[] {interface1});

        // 设置Field
        CtField field = CtField.make("private yuanjun.chen.proxy.Station station;", cc);

        cc.addField(field);

        CtClass stationClass = pool.get("yuanjun.chen.proxy.Station");
        ...
        cc.writeFile("D://testJavassit"); // 将class文件写到磁盘
    }
